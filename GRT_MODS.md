...
- `openpilot/grt/` — `__init__.py`, `registry.py`, `hooks.py`, `settings.py`, `tests/`
  ...
  - longitudinal comfort: `e2e_floor.py` (hook 6), `accel_ramp.py` (hooks 7 + 9),
    `hold_speed.py` (hook 8)
  - `cruise_log.py` — **TEMPORARY** diagnostic recorder, writes
    `<GRT_CONFIG_DIR>/cruise_log.csv`. Changes no behaviour. Delete once the set-speed
    stepping question is closed; see captains_log 2026-08-20.
