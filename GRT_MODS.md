...
| `openpilot/selfdrive/car/card.py` | import block; `SubMaster(...)`; after `initialize_v_cruise` | **C** | `+ GRT_SUB_CARD` on the service list **with the same three ignore lists as plannerd**, plus one hook line `grt_hooks.track_set_speed(...)`. The hook MUST stay after `initialize_v_cruise` (which resets `v_cruise` on the engage edge) and before the `CS.vCruise` assignment. **Deliberately here and not in `cruise.py`**, contrary to the original plan: `update_v_cruise(CS, enabled, is_metric)` is called directly by `selfdrive/car/tests/test_cruise_speed.py` in five places, so changing its signature to pass a `SubMaster` would break an upstream test suite. card's own health checks are scoped (`all_checks(['carControl'])`), so `mapdOut` cannot invalidate `carOutput`. |
