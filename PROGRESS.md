...
- **Last action:** Root-caused the harsh curve approach from the 11:28 SAST drive of
  2026-08-05 (route `00000072`, mapd_debug session 56) and fixed it with
  `_ramp_curve_ceiling()` in `scc_map.py`. Deployed. Prior: Cut map curve speed 10% (`map_curve_target_lat_a` 2.5 → 2.025) and deployed the
...
- **Next step:** **DRIVE IT** and judge the curve APPROACH (corner speed itself was
  already judged OK on 08-05). Instrument `/data/media/0/mapd_debug.log`, which now logs
  both `map_curve_speed_kmh` (mapd's raw step) and `curve_ceiling_kmh` (after the
  descent limit) — the gap between them IS the shaping. Expect onset decel ≈ −0.5 m/s²
  instead of −1.2, and no frames at/below −1.15 attributable to a curve.
  If it now feels LATE rather than harsh, raise the descent rate — but `APPROACH_DECEL`
  is shared with hazards and speed limits, both separately drive-validated, so add a
  curve-specific constant rather than changing it. Prior: **DRIVE IT.** Two questions, one instrument (`/data/media/0/mapd_debug.log` —
